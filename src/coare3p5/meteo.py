...
def psiu_26(z_L):
    ...
    k = find(zet < 0)  # unstable
    x = (1 - 15 * zet[k]) ** 0.25
    psik = 2.0 * log((1.0 + x) / 2.0) + log((1.0 + x * x) / 2.0) - 2.0 * arctan(x) + 2.0 * arctan(1.0)
    x = (1 - 10.15 * zet[k]) ** 0.3333
    psic = 1.5 * log((1.0 + x + x**2) / 3.0) - sqrt(3.0) * arctan((1.0 + 2.0 * x) / sqrt(3.0))
    psic += 4 * arctan(1.0) / sqrt(3.0)
...
def Le_water(t, sal):
    ...
    a = [2.5008991412e06, -2.3691806479e03, 2.6776439436e-01, -8.1027544602e-03, -2.0799346624e-05]
