...
def coare35vn(
    u, t, q, ts, P=1015, Rs=150, Rl=370, zu=18, zt=18, zq=18, lat=45, zi=600, rain=None, cp=None, sigH=None, jcool=1
):
    """
    ...
    q = np.copy(np.asarray(q, dtype=float))
    ...
    if not np.all([t.size == len, q.size == len, ts.size == len]):
        raise ValueError("coare35vn: u, t, q, ts arrays of different length")
    ...
    test = [isinstance(zu, int) or isinstance(zu, float)]

    test.append(isinstance(zt, int) or isinstance(zt, float))

    test.append(isinstance(zq, int) or isinstance(zq, float))
    ...
    zrf_u = 0.2  # User defined reference heights
...
if __name__ == "__main__":
    ...
    A = coare35vn(u, ta, rh, ts, P=Pa, Rs=rs, Rl=rl, zu=16, zt=16, zq=16, lat=Lat, zi=ZI, rain=Rain, jcool=1)

    fnameA = os.path.join(path, "../../../../../../Downloads/noaa-psd-pyCOARE-a90c9a2/test_35_output_py_082020.txt")
